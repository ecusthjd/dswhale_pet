...
| 💸 Out of Rice | balance < 0.01 CNY, or the API says `is_available:false`; in Go mode: any usage window hits 100% |

### Two modes: which feed decides how fast she eats

Right-click → **Monitor (mode)**, or pick it in Settings — she only ever uses one feed, so the two
keys never mix:

| Mode | What it watches | "Out of Rice" |
| --- | --- | --- |
| DeepSeek balance | how fast the balance drops (CNY/hour) | balance at zero / API says unavailable |
| OpenCode Go usage | how fast the rolling window's remaining % drops (%/hour) | any of rolling / weekly / monthly at 100% |

Filling in only one API key switches to that feed automatically. In Go mode, once the quota runs out
she shows a **candy-pink countdown above her head** — `Refill in HH:MM:SS`, ticking down in real time
to the next window reset.
